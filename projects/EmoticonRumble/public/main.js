...
let game = {
    config: {
        baseHealth: 20,
        healthMultiplier: 2,
        resistanceModifier: 0.04,
        resistanceLimit: 0.9,
        lifeStealModifier: 0.05,
        rollAmount: 6,
        baseStatPoints: 20,
        ...
        playerMoveSpeed: 100,
        playerDamageMitigation: 0.1,
        spawnLevelSplit: 1.3,
        aiBaseSpeed: 1000,
        aiMaxSpeed: 50,
        aiBaseMoveSpeedModifier: 0.04,
        gameSpeed: 33,
        xTileCount: 8,
        yTileCount: 8,
    },
    ...
    createBoard: function () {
        ...
        if (game.config.xTileCount < 2) game.config.xTileCount = 2;
        if (game.config.yTileCount < 2) game.config.yTileCount = 2;

        game.board = Array(game.config.yTileCount)
            .fill(null)
            .map(() => new Array(game.config.xTileCount).fill(null));
    },
    ...
    createAIMoveTimer(e1) {
        let timer = {
            ...
            timer: null,
            tick(thisTimer) {
                if (thisTimer === undefined) thisTimer = this;

                if (!thisTimer.emoticon.inCombat) thisTimer.emoticon.moveToTarget();
                ...
                if (moveSpeed < game.config.aiMaxSpeed) moveSpeed = game.config.aiMaxSpeed;
    ...
    isEmpty(posX, posY) {
        if (posX < 0 || posX > game.config.xTileCount - 1 || posY < 0 || posY > game.config.yTileCount - 1) return null;

        if (game.board[posY][posX] === null) return true;
        else return false;
    },
    ...
    spawnEmoticon(level, e1) {
        ...
        if (game.aiMoveTimers.length < spawnLimit) {
            ...
            while (spawnAttempts > 0) {
                ...
                if (
                    game.isEmpty(posX, posY) &&
                    !game.isEmoticon(posX + 1, posY) &&
                    !game.isEmoticon(posX, posY + 1) &&
                    !game.isEmoticon(posX - 1, posY) &&
                    !game.isEmoticon(posX, posY - 1)
                ) {
                    if (e1 === undefined) e1 = new Emoticon();
                    if (level > 1) {
                    ...
                    if (!e1.player) game.createAIMoveTimer(game.board[posY][posX]);
                    break;
                }
                spawnAttempts--;
            }
    ...
    startingSpawn(type) {
        ...
        if (type === "player") game.spawnEmoticon(1, game.playerEmoticon);

        let startSpawnAmount = Math.floor((game.config.xTileCount + game.config.yTileCount) / 3);
        if (startSpawnAmount < 2) startSpawnAmount = 2;
    ...
    createPlayerLeaderboard() {
        ...
        if (leaderboardCookie[0] === "") leaderboardCookie = [];
    ...
    addToPlayerLeaderboard(e1) {
        ...
        newPlayerLeaderboard = newPlayerLeaderboard.sort((a, b) => {
            if (a.wins + a.level > b.wins + b.level) return -1;
            if (a.wins + a.level < b.wins + b.level) return 1;
            return 0;
        });
        ...
        for (let i = 0; i < newPlayerLeaderboard.length; i++) {
            newCookie +=
                newPlayerLeaderboard[i].emoticon +
                "," +
                newPlayerLeaderboard[i].health +
                "," +
                newPlayerLeaderboard[i].attack +
                "," +
                newPlayerLeaderboard[i].defence +
                "," +
                newPlayerLeaderboard[i].wins;
            if (i < newPlayerLeaderboard.length - 1) newCookie += ",";
        }
...
let display = {
    titleTimer: {
        ...
        tick() {
            ...
            if (game.playerEmoticon !== null) {
                $("#title").append(
                    `[Emoticon]&nbsp;&nbsp;&nbsp;&nbsp;❤️${game.playerEmoticon.stats.currentHealth}⚔️${game.playerEmoticon.stats.attack}🛡️${game.playerEmoticon.stats.defence}&nbsp;&nbsp;&nbsp;&nbsp;[Rumble]`,
                );
                titleTimerSpeed = 33;
            } else {
    ...
    applyLightMode() {
        ...
        $(".nav-button").hover(
            function () {
                $(this).css("background-color", "#b3e7ff");
            },
            function () {
                $(this).css("background-color", "#ccefff");
            },
        );
    ...
    drawBoard(ctx) {
        ...
        for (let y = 0; y < game.config.yTileCount; y++) {
            for (let x = 0; x < game.config.xTileCount; x++) {
                if (y % 2 === 0) {
                    if (x % 2 === 0) ctx.fillStyle = display.tileColor;
                    else ctx.fillStyle = display.backgroundColor;
                } else {
                    if (x % 2 !== 0) ctx.fillStyle = display.tileColor;
                    else ctx.fillStyle = display.backgroundColor;
                }
                ...
                if (game.isEmoticon(x, y)) {
                    ...
                    if (game.playerEmoticon === null) {
                        if (e1.level > 4) {
                            ...
                            if (e1.level > 9) ctx.strokeStyle = "#3399ff";
                            if (e1.level > 14) ctx.strokeStyle = "#ff66ff";
                            if (e1.level > 19) ctx.strokeStyle = "#ff3333";
                        ...
                        if (e1.player) {
                            ...
                            if (e1.level >= game.playerEmoticon.level) {
                                ...
                                if (!display.darkMode) ctx.strokeStyle = `rgba(${55 * intensity}, ${155 * intensity}, ${255 * intensity})`;
                                else ctx.strokeStyle = `rgba(${85 * intensity}, ${185 * intensity}, ${285 * intensity})`;
                            }
                            if (e1.level < game.playerEmoticon.level) {
                                let intensity = 1 - (game.playerEmoticon.level - e1.level) / 15;
                                if (intensity < 0 || e1.level <= game.playerEmoticon.level - 15) intensity = 0;
                                if (!display.darkMode) ctx.strokeStyle = `rgba(55, 155, 255, ${intensity})`;
                                else ctx.strokeStyle = `rgba(85, 185, 285, ${intensity})`;
                            }
                            ctx.strokeRect(x * display.tileSize + display.offsetX, y * display.tileSize + display.offsetY, display.tileSize, display.tileSize);
                        }
                    }
                    ...
                    ctx.fillText(
                        stats,
                        x * display.tileSize + display.offsetX + (display.tileSize / 2 - ctx.measureText(stats).width / 2),
                        y * display.tileSize + display.offsetY + Math.floor(display.tileSize / 3.3),
                    );
                    //Emoticon
                    ctx.font = "16px Verdana";
                    ctx.fillText(
                        e1.emoticon,
                        x * display.tileSize + display.offsetX + (display.tileSize / 2 - ctx.measureText(e1.emoticon).width / 2),
                        y * display.tileSize + display.offsetY + Math.floor(display.tileSize / 1.8),
                    );
                    //Level
                    ...
                    ctx.fillText(
                        level,
                        x * display.tileSize + display.offsetX + (display.tileSize / 2 - ctx.measureText(level).width / 2),
                        y * display.tileSize + display.offsetY + Math.floor(display.tileSize / 1.25),
                    );
                }
                //Walls
                if (game.isWall(x, y)) {
                    ...
                    ctx.fillText(
                        wall.emoji,
                        x * display.tileSize + display.offsetX + (display.tileSize / 2 - ctx.measureText(wall.emoji).width / 2),
                        y * display.tileSize + display.offsetY + Math.floor(display.tileSize / 1.4),
                    );
                }
            }
        }
                ...
                if (game.isEmoticon(x, y)) {
                    ...
                    if (e1.attackDirection !== null) {
                        ...
                        if (e1.attackDirection === "N")
                            ctx.fillText(
                                "⚔️",
                                x * display.tileSize + display.offsetX + (display.tileSize / 2 - ctx.measureText("⚔️").width / 2),
                                y * display.tileSize + display.offsetY + Math.floor(display.tileSize / 10),
                            );
                        if (e1.attackDirection === "S")
                            ctx.fillText(
                                "⚔️",
                                x * display.tileSize + display.offsetX + (display.tileSize / 2 - ctx.measureText("⚔️").width / 2),
                                y * display.tileSize + display.offsetY + display.tileSize + Math.floor(display.tileSize / 10),
                            );
                        if (e1.attackDirection === "E")
                            ctx.fillText(
                                "⚔️",
                                x * display.tileSize + display.offsetX + (display.tileSize - ctx.measureText("⚔️").width / 2),
                                y * display.tileSize + display.offsetY + Math.floor(display.tileSize / 1.8),
                            );
                        if (e1.attackDirection === "W")
                            ctx.fillText(
                                "⚔️",
                                x * display.tileSize + display.offsetX + (0 - ctx.measureText("⚔️").width / 2),
                                y * display.tileSize + display.offsetY + Math.floor(display.tileSize / 1.8),
                            );
                    }
                    //Combat Display
                    if (e1.combatDisplay !== null) {
                        ...
                        if (e1.attackDirection !== null || e1.attackerDirection !== null) {
                            ...
                            if (e1.attackDirection === "N" || e1.attackerDirection === "N") {
                                ctx.strokeText(
                                    text,
                                    x * display.tileSize + display.offsetX + (display.tileSize - ctx.measureText(text).width / 2) + 25,
                                    y * display.tileSize + display.offsetY + display.tileSize / 1.7,
                                );
                                ctx.fillText(
                                    text,
                                    x * display.tileSize + display.offsetX + (display.tileSize - ctx.measureText(text).width / 2) + 25,
                            ...
                            if (e1.attackDirection === "S" || e1.attackerDirection === "S") {
                                ctx.strokeText(
                                    text,
                                    x * display.tileSize + display.offsetX + (0 - ctx.measureText(text).width / 2) - 25,
                                    y * display.tileSize + display.offsetY + display.tileSize / 1.7,
                                );
                                ctx.fillText(
                                    text,
                            ...
                            if (e1.attackDirection === "E" || e1.attackerDirection === "E") {
                                ctx.strokeText(
                                    text,
                                    x * display.tileSize + display.offsetX + (display.tileSize / 2 - ctx.measureText(text).width / 2),
                                    y * display.tileSize + display.offsetY + display.tileSize + display.tileSize / 10 + 15,
                                );
                                ctx.fillText(
                                    text,
                                    x * display.tileSize + display.offsetX + (display.tileSize / 2 - ctx.measureText(text).width / 2),
                            ...
                            if (e1.attackDirection === "W" || e1.attackerDirection === "W") {
                                ctx.strokeText(
                                    text,
                                    x * display.tileSize + display.offsetX + (display.tileSize / 2 - ctx.measureText(text).width / 2),
                                    y * display.tileSize + display.offsetY + display.tileSize / 10 - 15,
                                );
                                ctx.fillText(
                                    text,
                                    x * display.tileSize + display.offsetX + (display.tileSize / 2 - ctx.measureText(text).width / 2),
        ...
        if (game.gameOverState) {
            ...
            if (game.playerEmoticon.level % 2 === 0) gameOverText = "☠️";

            ctx.font = "bold " + Math.floor((ctx.canvas.clientWidth + ctx.canvas.clientHeight) / 10) + "px Verdana";
            ctx.fillText(
                gameOverText,
                ctx.canvas.clientWidth / 2 - ctx.measureText(gameOverText).width / 2,
                ctx.canvas.clientHeight / 2 + ctx.canvas.clientHeight / 10,
            );
        }
    },

    drawLeaderboard() {
        if (document.getElementById("canvas") !== null) $("#leaderboard").css("min-width", $("#canvas").css("width"));
        else $("#leaderboard").css("width", "initial");
    ...
    drawAIRumbleButtons() {
      ...
      <span id="faster-button" class="game-button"><span>🐇</span></span>`,
        ...
        if (display.darkMode) display.applyDarkMode();
        else display.applyLightMode();

        $("#slower-button").click(function () {
            game.aiSpeed += 100;
            if (game.aiSpeed > 100 && game.aiSpeed < 200) game.aiSpeed = 100;
        });
        $("#faster-button").click(function () {
            if (game.aiSpeed < 200) game.aiSpeed = game.config.aiMaxSpeed;
            else game.aiSpeed -= 100;
        });
    ...
    drawCreateChar() {
      ...
      <div class="nav-button" id="create-char-btn"><span>Create Character</span></div>`,
        ...
        $("#emoticon-input").on("input", function () {
            ...
            if (totalSpent >= game.config.baseStatPoints && $("#emoticon-input").val().length > 0) $("#create-char-btn").removeClass("btn-disable");
            else $("#create-char-btn").addClass("btn-disable");
        });
        ...
        $("#char-h").on("input", function () {
            ...
            if (totalSpent >= game.config.baseStatPoints) {
                ...
                if ($("#emoticon-input").val().length > 0) $("#create-char-btn").removeClass("btn-disable");
            } else {
    ...
    drawGame() {
        if (document.getElementById("canvas") === null) {
            $("#display").append(
                `<canvas id="canvas" width="${game.config.xTileCount * display.tileSize + display.offsetX * 2}" height="${game.config.yTileCount * display.tileSize + display.offsetY * 2}">`,
            );
...
class Emoticon {
    constructor(emoticon, health, attack, defence) {
        if (emoticon === undefined) this.emoticon = createEmoticon();
        else this.emoticon = emoticon;
        ...
        } else this.stats = createStats();

        this.level =
            (this.stats.health - game.config.baseHealth) / game.config.healthMultiplier +
            this.stats.attack +
            this.stats.defence -
            (game.config.baseStatPoints - 1);
        this.wins = 0;
        this.target = null;
        this.inCombat = false;
    ...
    levelUp(e2) {
        ...
        for (let i = 0; i < points; i++) {
            ...
            if (rand === 0) this.stats.health += 1 * game.config.healthMultiplier;
            else if (rand === 1) this.stats.attack++;
            else if (rand === 2) this.stats.defence++;
        ...
        if (this.player) {
            ...
            if (game.aiSpeed < 300) game.aiSpeed = 300;
        }
    }
    ...
    lifeSteal(e2) {
        ...
        if (this.level > e2.level) {
            ...
            if (this.stats.currentHealth + heal > this.stats.health) heal = this.stats.health - this.stats.currentHealth;
        ...
        if (this.player) {
            if (this.combatDisplay === "🎲") this.combatDisplay = `🎲+${heal}❤️`;
            else this.combatDisplay = `+${heal}❤️`;
            ...
            clearCombatDisplayTimer.timer = window.setTimeout(
                function (timer, player) {
                    game.removeFightTimer(timer);
                    player.combatDisplay = null;
                },
                game.config.playerFightSpeed * 1.3,
                clearCombatDisplayTimer,
                this,
            );
    ...
    remove() {
        ...
        if (pos !== null) game.board[pos[1]][pos[0]] = null;
    }
        ...
        for (let y = 0; y < game.config.yTileCount; y++) {
            for (let x = 0; x < game.config.xTileCount; x++) {
                if (game.isEmoticon(x, y)) {
                    ...
                    if (pos !== null) {
                        if (x !== pos[0] && y !== pos[1]) {
                            if (game.board[y][x].level >= this.level) if (!game.board[y][x].inCombat) potentialTargets.push(game.board[y][x]);
                        }
                    }
                }
            }
        }
        if (potentialTargets.length > 0) this.target = potentialTargets[getRandomInt(potentialTargets.length)];
        else this.target = null;
    }

    moveToTarget() {
        if (this.target === null) this.findTarget();
        else if (game.findEmoticon(this.target) === null) this.findTarget();
        ...
        if (pos !== null) {
            if (this.target !== null) {
                ...
                if (pos[0] === targetPos[0]) {
                    if (pos[1] < targetPos[1]) moveY += 1;
                    else moveY -= 1;
                } else if (pos[1] === targetPos[1]) {
                    if (pos[0] < targetPos[0]) moveX += 1;
                    else moveX -= 1;
                } else {
                ...
                if (game.isEmpty(moveX, moveY)) {
                    ...
                    if (!game.board[moveY][moveX].inCombat) {
                        if (pos[0] - moveX > 0) this.attackDirection = "W";
                        if (pos[0] - moveX < 0) this.attackDirection = "E";
                        if (pos[1] - moveY > 0) this.attackDirection = "N";
                        if (pos[1] - moveY < 0) this.attackDirection = "S";
        ...
        function moveRandom(pos, e1) {
            ...
            let moveX = pos[0];
            let moveY = pos[1];
            if (xOrY) moveX = moves[getRandomInt(2)] + pos[0];
            else moveY = moves[getRandomInt(2)] + pos[1];
    ...
    move(direction) {
        if (!this.inCombat) {
            ...
            else if (direction === "E") moveX++;
            else if (direction === "N") moveY--;
            else if (direction === "S") moveY++;
...
function fight(e1, e2) {
    ...
    if (e1.attackDirection === "N") e2.attackerDirection = "S";
    if (e1.attackDirection === "S") e2.attackerDirection = "N";
    if (e1.attackDirection === "E") e2.attackerDirection = "W";
    if (e1.attackDirection === "W") e2.attackerDirection = "E";
    ...
    let aiFight = function (e1, e2, fightTimer) {
        ...
        while (true) {
            ...
            if (status) {
                ...
                fightOutcomeTimer.timer = window.setTimeout(
                    function (timer) {
                        game.removeFightTimer(timer);
    ...
    let playerFight = function (e1, e2, thisFight, fightTimer) {
        if (fightTimer) game.removeFightTimer(fightTimer);
        ...
        if (status) {
            ...
            clearCombatDisplayTimer.timer = window.setTimeout(
                ...
                game.config.playerFightSpeed * 0.9,
                clearCombatDisplayTimer,
            );
            game.fightTimers.push(clearCombatDisplayTimer);
        }
...
function attack(e1, e2) {
    ...
    if (resistance > game.config.resistanceLimit) resistance = game.config.resistanceLimit;
...
function fightOutcome(e1, e2, status) {
    if (status === "draw") {
        if (getRandomInt(2)) {
            if (e1.player) e1.combatDisplay = "🎲";
            status = "win";
        } else {
            if (e2.player) e2.combatDisplay = "🎲";
            status = "lose";
        }
    }
    if (status === "win") {
        if (e2.player) game.gameOver();

        e1.wins++;
        e1.levelUp(e2);
        e1.lifeSteal(e2);

        if (e2.level > 1) game.leaderboard.push(e2);
        e2.remove();

        if (e2.level > 1) game.spawnEmoticon(Math.ceil(e2.level / game.config.spawnLevelSplit));
    ...
    if (status === "lose") {
        if (e1.player) game.gameOver();

        e2.wins++;
        e2.levelUp(e1);
        e2.lifeSteal(e1);

        if (e1.level > 1) game.leaderboard.push(e1);
        e1.remove();

        if (e1.level > 1) game.spawnEmoticon(Math.ceil(e1.level / game.config.spawnLevelSplit));
...
function createStats() {
    ...
    for (let i = 0; i < game.config.baseStatPoints; i++) {
        ...
        let rand = getRandomInt(3);
        if (rand === 0 && health < game.config.baseMaxHealthPoints) health++;
        else if (rand === 1 && attack < game.config.baseMaxAttackPoints) attack++;
        else if (rand === 2 && defence < game.config.baseMaxDefencePoints) defence++;
        else i--;
    }
    health = health * game.config.healthMultiplier + game.config.baseHealth;
