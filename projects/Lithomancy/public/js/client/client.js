import { SHAPE_SIDES, classifyCast } from "../shared/game.js";
// ─── Connection ───────────────────────────────────────────────────────────────
const PARTYKIT_HOST = location.hostname === "localhost" ? "localhost:1999" : "lithomancy.ectoboi.partykit.dev";
let ws = null;
...
const comboOverlayWrap = document.getElementById("combo-overlay-wrap");
...
const comboOpen = document.getElementById("combo-open");
...
function beep(when, freq, duration, type, gain) {
    if (!audioCtx) return;
...
function beepSweep(when, fromFreq, toFreq, duration, type, gain) {
    if (!audioCtx) return;
    const osc = audioCtx.createOscillator();
    const amp = audioCtx.createGain();
    osc.type = type;
...
function noiseBurst(when, duration, gain, hp = 700) {
    if (!audioCtx) return;
...
function filteredNoiseBurst(when, duration, gain, type, cutoff, q = 0.7) {
    if (!audioCtx) return;
    const len = Math.max(1, Math.floor(audioCtx.sampleRate * duration));
    const buf = audioCtx.createBuffer(1, len, audioCtx.sampleRate);
    const data = buf.getChannelData(0);
...
function attachButtonSfx(btn) {
    if (!btn) return;
    btn.addEventListener("click", () => playSfx("ui_click"));
}
function getBoardDiff(before, after) {
    const changed = [];
...
function playActionSfxFromStateDiff(prev, next) {
    if (!prev) return;
...
function playOutcomeSfxFromState(prev, next) {
...
function connectToParty(code) {
    const protocol = location.protocol === "https:" ? "wss" : "ws";
    ws = new WebSocket(`${protocol}://${PARTYKIT_HOST}/parties/main/${code}`);
...
const TURRET_R = 24;
const STONE_R = 26;
const TURRET_RUNES = ["ᚠ", "ᚱ", "ᚧ", "ᚩ", "ᚥ", "ᚬ", "ᚸ", "ᚻ"];
// Turret positions: 8 around a circle, 0 at top, clockwise
function turretPos(i) {
...
function getHandPositions(isBottomPlayerView) {
...
function drawPolygon(x, y, sides, r) {
    ctx.beginPath();
...
function drawStarCycles(x, y, points, step, r, rotation) {
...
function drawStarFace(x, y, value, r) {
    ctx.save();
...
function drawStone(x, y, face, selected, dim) {
    ctx.save();
...
function drawTurret(i, owner, highlighted = false) {
...
function drawRefArrow(x1, y1, x2, y2) {
    const dx = x2 - x1,
        dy = y2 - y1;
...
function drawOverlayRefArrow(c, x1, y1, x2, y2) {
    const dx = x2 - x1,
        dy = y2 - y1;
    const len = Math.hypot(dx, dy);
    const ux = dx / len,
...
function drawComboOverlay() {
    ...
    const triR = 72;
    const spell = [cx, triCY - triR];
    const potion = [cx + triR * Math.cos(Math.PI / 2 + (2 * Math.PI) / 3), triCY - triR * Math.sin(Math.PI / 2 + (2 * Math.PI) / 3)];
    const charm = [cx + triR * Math.cos(Math.PI / 2 - (2 * Math.PI) / 3), triCY - triR * Math.sin(Math.PI / 2 - (2 * Math.PI) / 3)];
...
function drawWinReference() {
    ...
    const spell = [ox, oy - 26];
    const potion = [ox - 38, oy + 38];
    const charm = [ox + 38, oy + 38];
...
function drawHandLabel(hand, isMyHand) {
...
let selectedStones = new Set();
let myHandForRecast = null;
function render() {
